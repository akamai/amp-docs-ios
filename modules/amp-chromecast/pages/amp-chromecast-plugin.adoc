...
:external-sdk-url: https://developers.google.com/cast/docs/ios_sender#manual_setup
:not-tvos: true
...
In your `AppDelegate` class, the receiver ID must be specified.
[source,swift]
----
...
For starting the video cast to a Chromecast device, just add your own button and assign it the custom class `GCKUICastButton`, that way video cast will start the flow automatically.
